...
//! into program nodes lives in `crate::compile`, so the semantic layer depends
...
pub(crate) enum FnStmt {
    ...
    /// compile time; no program node is emitted and execution is deferred
    /// to each use site.
    ...
    Cd(Template),
    /// `env(KEY = value; ...) { body }`: export `pairs` to the command
    /// subprocess environment for the duration of `body`.
    EnvBlock {
        pairs: Vec<EnvPair>,
        body: Vec<FnStmt>,
    },
