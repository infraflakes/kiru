...
use crate::ir::{
    Arm, ArmPattern, Call, EnvPair, Instruction, Ir, Project, Segment, Sync, Template as IrTemplate,
};
use crate::subprocess;
use crate::syntax::lexer::Lexer;
use crate::syntax::source::ArmPattern as DslArmPattern;
use crate::syntax::{Part as DslPart, Program, ProjectField, Stmt, Template, TopLevel};
...
fn lower_template(tmpl: &Template) -> IrTemplate {
    IrTemplate {
        segments: tmpl
            .parts
            .iter()
            .map(|p| match p {
                ...
                DslPart::Cmd(inner) => Segment::Cmd(lower_template(inner)),
            })
            .collect(),
    }
}
...
fn build_ir(state: LoweringState) -> Result<Ir, CompileError> {
