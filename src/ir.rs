...
pub fn write_template(tmpl: &Template) -> String {
    ...
    for segment in &tmpl.segments {
        match segment {
            Segment::Lit(s) => out.push_str(&format!(" (lit {})", quote_string(s))),
            Segment::Cmd(inner) => out.push_str(&format!(" (cmd {})", write_template(inner))),
        }
    }
    out.push(')');
    out
...
pub fn render_ir_literal(tmpl: &Template) -> String {
    ...
    for segment in &tmpl.segments {
        match segment {
            ...
            Segment::Cmd(inner) => out.push_str(&render_ir_literal(inner)),
        }
    }
    out
}
...
mod tests {
    ...
    fn sample_ir() -> Ir {
        let check_cmd = Template {
            segments: vec![Segment::Cmd(Template::lit("test -f $HOME"))],
        };
